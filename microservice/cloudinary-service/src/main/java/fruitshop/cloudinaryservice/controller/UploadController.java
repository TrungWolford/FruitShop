package fruitshop.cloudinaryservice.controller;

import fruitshop.cloudinaryservice.service.CloudinaryService;
...
public class UploadController {
    ...
    public ResponseEntity<?> getFileInfo(@RequestParam("publicId") String publicId) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("success", false, "error", "Lỗi lấy thông tin file: " + e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("success", false, "error", "Đã xảy ra lỗi không mong muốn"));
        }
