...
public class AdminProductPage {
  ...
  // Pagination
  private By previousPageButton = By
      .xpath("//button[contains(@aria-label, 'Previous') or .//svg[contains(@class, 'ChevronLeft')]]");
  private By nextPageButton = By
      .xpath("//button[contains(@aria-label, 'Next') or .//svg[contains(@class, 'ChevronRight')]]");
  ...
  public void selectCategory(String categoryName) {
    try {
      ...
      java.util.List<WebElement> badges = driver.findElements(
          By.xpath(
              "//div[@data-state='open']//div[contains(@class, 'cursor-pointer') and contains(@class, 'bg-white')]"));
    ...
    try {
      ...
      for (int i = 0; i < comboboxButtons.size(); i++) {
        ...
        try {
          ...
          System.out.println(
              "Combobox " + i + ": text='" + text + "', data-state=" + dataState + ", aria-expanded=" + ariaExpanded);
        } catch (Exception e) {
