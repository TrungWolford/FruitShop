...
const EditProductModal: React.FC<EditProductModalProps> = ({ isOpen, onClose, onSuccess, product }) => {
  ...
  const handleSingleImageChange = async (file: File, slotIndex: number) => {
    ...
    try {
      ...
      const hasExistingImage = product.images && product.images[slotIndex] && !removedExistingImages.has(slotIndex);
      ...
      toast.success(hasExistingImage ? `Đã thay thế hình ảnh ở ô ${slotIndex + 1}` : `Đã thêm hình ảnh vào ô ${slotIndex + 1}`);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`${SUCCESS_MESSAGES.PRODUCT_UPDATED} ${validImageNames.length > 0 ? `với ${validImageNames.length} hình ảnh` : ''}`);
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-5 gap-4">
                {Array.from({ length: 5 }, (_, index) => {
                  ...
                  const hasExistingImage = product.images && product.images[index] && !removedExistingImages.has(index);
