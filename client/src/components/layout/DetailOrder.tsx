                ...
                <div className="space-y-3 text-sm">
                  ...
                  {order.deliveryDate && (
                    <div className="flex justify-between items-center py-2 border-b border-gray-100">
                      ...
                      <span className="font-semibold text-gray-900">{formatDate(order.deliveryDate)}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center py-2">
                    <span className="text-gray-600 font-medium">Phương thức thanh toán:</span>
                    <span className="font-semibold text-gray-900">{getPaymentMethodText(order.paymentMethod)}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <CardContent className="p-4">
              ...
              <div className="space-y-4">
                {order.items.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 p-4 border-2 border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-gray-900 text-lg mb-1">{item.product.productName}</h4>
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="text-sm text-gray-600 mb-1">Số lượng: <span className="font-semibold">{item.quantity}</span></p>
                      <p className="text-sm text-gray-600 mb-1">Đơn giá: <span className="font-semibold">{formatPrice(item.price)}</span></p>
