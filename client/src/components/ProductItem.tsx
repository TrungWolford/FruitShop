...
const ProductItem: React.FC<ProductItemProps> = ({ product, onAddToCart, onAddToWishlist }) => {
  ...
  const handleProductClick = () => {
    ...
    const productSlug = `${product.productId}-${product.productName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}`;
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // Thử thêm sản phẩm trực tiếp - CartService sẽ tự tạo cart nếu cần
