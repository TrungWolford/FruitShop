...
const HistoryReceipt: React.FC = () => {
          ...
          order.orderDetails.forEach((detail, detailIndex) => {
            ...
            if (detail.productImages && detail.productImages.length > 0) {
              detail.productImages.forEach((imageUrl, imageIndex) => {
                console.log(`    🖼️ Image ${imageIndex + 1}: "${imageUrl}" -> "${getImageUrl(imageUrl)}"`);
              });
            }
          });
        });
  ...
  const getStatusColor = (status: number) => {
    switch (status) {
      case 0: return 'bg-red-600 text-white border-red-600'; // Huy
      case 1: return 'bg-blue-600 text-white border-blue-600'; // Dang van chuyen  
      case 2: return 'bg-green-600 text-white border-green-600'; // Da hoan thanh
  ...
  const getProgressStep = (status: number) => {
    switch (status) {
      case 0: return 0; // Huy
      case 1: return 2; // Dang van chuyen
      case 2: return 4; // Da hoan thanh
      default: return 0;
    }
  };
  ...
  const getCurrentStatusColor = (status: number) => {
    switch (status) {
      case 0: return 'bg-red-500'; // Huy
      case 1: return 'bg-blue-500'; // Dang van chuyen
      case 2: return 'bg-green-500'; // Da hoan thanh
              ...
              <div className="flex gap-4 flex-1">
                <div className="relative max-w-xs">
                  ...
                  />
                      ...
                      <Button variant="outline" className="flex items-center gap-2  border-2 border-gray-200 hover:bg-gray-50">
                        <span>
                          {statusFilter === 'all' ? 'Tất cả trạng thái' : 
                           statusFilter === '0' ? 'Đã hủy' :
                           statusFilter === '1' ? 'Đang vận chuyển' :
                           statusFilter === '2' ? 'Đã hoàn thành' : 'Tất cả trạng thái'}
                        </span>
                      ...
                      {order.status !== 0 && (
                        <div className="mb-6">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium text-gray-700">Tiến trình đơn hàng</span>
                          </div>
                                ...
                                <div className="flex-1 h-2 bg-gray-200  overflow-hidden">
                                  <div 
                                    className={`h-full transition-all duration-500 ${
                                      currentStep >= 1 ? getCurrentStatusColor(order.status) : 'bg-gray-300'
                                    }`}
                                    style={{ width: `${Math.max(0, Math.min(100, (currentStep / 4) * 100))}%` }}
                                  ></div>
                                </div>
                             </div>
                             ...
                             <div className="flex justify-between mt-2">
                               <div className="flex flex-col items-center">
                                 <div className={`w-6 h-6  flex items-center justify-center text-xs font-medium ${
                                   currentStep >= 1 ? getProgressColor(currentStep, 1) + ' text-white' : 'bg-gray-300 text-gray-600'
                                 }`}>
                                   {currentStep >= 1 ? <CheckCircle className="w-4 h-4" /> : '1'}
                                 </div>
                                 <span className="text-xs text-gray-600 mt-1">Chờ xác nhận</span>
                               </div>
                               <div className="flex flex-col items-center">
                                 <div className={`w-6 h-6  flex items-center justify-center text-xs font-medium ${
                                   currentStep >= 2 ? getProgressColor(currentStep, 2) + ' text-white' : 'bg-gray-300 text-gray-600'
                                 }`}>
                                   {currentStep >= 2 ? <CheckCircle className="w-4 h-4" /> : '2'}
                                 </div>
                                 <span className="text-xs text-gray-600 mt-1">Đã xác nhận</span>
                               </div>
                               <div className="flex flex-col items-center">
                                 <div className={`w-6 h-6  flex items-center justify-center text-xs font-medium ${
                                   currentStep >= 3 ? getProgressColor(currentStep, 3) + ' text-white' : 'bg-gray-300 text-gray-600'
                                 }`}>
                                   {currentStep >= 3 ? <CheckCircle className="w-4 h-4" /> : '3'}
                                 </div>
                                 <span className="text-xs text-gray-600 mt-1">Đang giao</span>
                               </div>
                               <div className="flex flex-col items-center">
                                 <div className={`w-6 h-6  flex items-center justify-center text-xs font-medium ${
                                   currentStep >= 4 ? getProgressColor(currentStep, 4) + ' text-white' : 'bg-gray-300 text-gray-600'
                                 }`}>
                                   {currentStep >= 4 ? <CheckCircle className="w-4 h-4" /> : '4'}
                                 </div>
                                 <span className="text-xs text-gray-600 mt-1">Đã giao</span>
                               </div>
                             </div>
                          </div>
                        </div>
                      ...
                      {/* Order Items Preview */}
                          <div>
                            ...
                            <div className="space-y-3">
                              ...
                              {Array.from({ length: Math.min(3, order.orderDetails.length) }, (_, index) => {
                                const detail = order.orderDetails[index];
                                return (
                                  <div key={detail?.orderDetailId || `placeholder-${index}`} className="flex items-center gap-4 p-4 bg-gray-50">
                                    <div className="relative w-20 h-20 bg-gradient-to-br from-gray-100 to-gray-200 border flex items-center justify-center overflow-hidden shadow-sm">
                                      {detail && detail.productImages && detail.productImages.length > 0 ? (
                                        <>
                                          <img 
                                  ...
                                  onError={(e) => {
                                              console.log('❌ Image failed to load:', getImageUrl(detail.productImages[0]));
                                    ...
                                    target.style.display = 'none';
                                            ...
                                            onLoad={() => {
                                              console.log('✅ Image loaded successfully:', getImageUrl(detail.productImages[0]));
                                            }}
                                          />
                            ...
                            <div className="flex-1">
                                      <h4 className="font-medium text-gray-900 text-sm">{detail?.productName || 'Sản phẩm không xác định'}</h4>
                                      <p className="text-xs text-gray-600">
                                        Số lượng: {detail?.quantity || 0} x {formatPrice(detail?.unitPrice || 0)}
                              </p>
                            </div>
                            <div className="text-right">
                                      <p className="font-semibold text-gray-900 text-sm">{formatPrice(detail?.totalPrice || 0)}</p>
                            </div>
                          </div>
                                );
                              })}
                            ...
                            <div>
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="p-4 bg-gray-50">
                                  <h4 className="font-medium text-gray-900 text-sm mb-1">Người nhận</h4>
                                ...
                                {/* Phone Number */}
                                <div className="p-4 bg-gray-50">
                                  <h4 className="font-medium text-gray-900 text-sm mb-1">Số điện thoại</h4>
                                  <p className="text-sm text-gray-600">
                                ...
                                {/* Address */}
                                <div className="p-4 bg-gray-50">
                                  <h4 className="font-medium text-gray-900 text-sm mb-1">Địa chỉ giao hàng</h4>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-semibold text-red-600">{formatPrice(selectedOrder.totalAmount)}</p>
                </div>
              </div>
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="font-bold text-gray-900">{selectedOrder.shipping.receiverName}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="font-bold text-gray-900">{selectedOrder.shipping.receiverPhone}</p>
                            </div>
                          </div>
                        </div>
                        <div className="space-y-3">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {selectedOrder.orderDetails.map((detail) => (
                    <div key={detail.orderDetailId} className="flex items-center gap-4 p-4 border bg-white shadow-sm hover:shadow-md transition-shadow">
                      <div className="relative w-20 h-20 bg-gradient-to-br from-gray-100 to-gray-200 border flex items-center justify-center overflow-hidden shadow-sm">
                        {detail.productImages && detail.productImages.length > 0 ? (
                          <>
                            <img 
                              ...
                              onError={(e) => {
                                console.log('❌ Detail modal image failed to load:', getImageUrl(detail.productImages[0]));
                                const target = e.target as HTMLImageElement;
                                const fallback = target.nextElementSibling as HTMLElement;
                                target.style.display = 'none';
                                if (fallback) fallback.classList.remove('hidden');
                              ...
                              onLoad={() => {
                                console.log('✅ Detail modal image loaded successfully:', getImageUrl(detail.productImages[0]));
                              }}
                            />
                            <div className="hidden absolute inset-0 flex items-center justify-center bg-gradient-to-br from-blue-100 to-blue-200">
