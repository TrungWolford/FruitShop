  ...
  const loadProducts = async (page: number = 0) => {
    try {
      ...
      if (response.content) {
        ...
        console.log('📸 Products with images:', response.content.map((p: any) => ({
          id: p.productId,
          name: p.productName,
          images: p.images
        })))
        ...
        console.log('📸 Products with images:', response.map((p: any) => ({
          id: p.productId,
          name: p.productName,
          images: p.images
        })))
              ...
              <div className="flex gap-2 flex-1">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" className="w-40 border border-gray-300 bg-gray-50 text-gray-900 hover:bg-gray-100 rounded-md flex items-center justify-between">
          ...
          <div className="bg-white shadow-sm border border-gray-200 overflow-hidden">
              ...
              <Table className="w-full">
                <TableHeader>
                  <TableRow className="bg-slate-800 hover:bg-slate-800 border-b border-slate-700">
                    <TableHead className="font-semibold text-white px-4 py-3 text-center text-sm w-24">ID</TableHead>
                    <TableHead className="font-semibold text-white px-4 py-3 text-left text-sm w-20">Hình ảnh</TableHead>
                    <TableHead className="font-semibold text-white px-4 py-3 text-left text-sm w-48">Tên sản phẩm</TableHead>
                    <TableHead className="font-semibold text-white px-4 py-3 text-left text-sm w-32">Tác giả</TableHead>
                    <TableHead className="font-semibold text-white px-4 py-3 text-center text-sm w-28">Giá</TableHead>
                    <TableHead className="font-semibold text-white px-4 py-3 text-center text-sm w-20">Tồn kho</TableHead>
                    <TableHead className="font-semibold text-white px-4 py-3 text-center text-sm w-24">Trạng thái</TableHead>
                    <TableHead className="font-semibold text-white px-4 py-3 text-center text-sm w-28">Ngày tạo</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? (
                    ...
                    >
                      <TableCell className="px-4 py-3 select-none">
                        <div className="flex justify-center">
                          <div className="px-2 py-1 bg-amber-100 rounded flex items-center justify-center">
                            <span className="text-xs font-bold text-amber-600">#{product.productId}</span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="px-4 py-3 select-none">
                        <div className="flex items-center">
                          {(() => {
                            console.log('🖼️ Rendering image for product:', product.productName)

                            console.log('🖼️ Product images field:', (product as any).images)
                            ...
                            if (imagesArray && imagesArray.length > 0) {
                              ...
                              console.log('❌ No images found for product:', product.productName)
                              return (
                      ...
                      <TableCell className="px-4 py-3 select-none">
                        <div className="font-medium text-gray-700 text-sm">{product.author}</div>
                      </TableCell>
                      <TableCell className="px-4 py-3 select-none">
                        <div className="flex justify-center">
                      ...
                      <TableCell className="px-4 py-3 select-none">
                        <div className="flex justify-center">
                          <span className={`font-bold text-sm ${
                            product.stock > 50 ? 'text-green-700' : 
            ...
            {!loading && currentProducts.length === 0 && (
              <div className="text-center py-12 px-6">
                ...
                <p className="text-gray-500 mb-3 text-sm">Vui lòng thử lại với từ khóa khác hoặc thêm sản phẩm mới</p>
                <Button 
                  onClick={() => setIsAddProductModalOpen(true)}
