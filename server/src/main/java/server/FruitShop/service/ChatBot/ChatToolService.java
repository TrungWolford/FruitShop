...
public class ChatToolService {
  ...
  public String getProductDetail(String productId) {
    try {
      ...
      if (product == null)
        return errorJson("Không tìm thấy sản phẩm với ID: " + productId);
   ...
   * Tạo bản ghi shipping template (không gắn với đơn hàng nào) và trả về
   * shippingId
   ...
   * @return JSON string: {"shippingId": "...", "receiverName": ...,
   *         "shippingFee": ...}
   */
   ...
   *
   * @param accountId     ID tài khoản của user
   * @param itemsJson     JSON string danh sách sản phẩm, VD:
  ...
  public String createOrderFromChat(String accountId, String itemsJson,
    ...
    try {
      ...
      // Nếu shippingId rỗng (Gemini gọi createOrderFromChat cùng round với
   ...
   * @return JSON string: {"cartItemId": "...", "productName": "...", "quantity":
   *         N, ...}
   */
  ...
  public String createRefundFromChat(String accountId, String orderId,
    ...
    try {
      ...
      result.put("message",
          "Yêu cầu hoàn trả đã được gửi thành công. Shop sẽ xem xét và phản hồi trong thời gian sớm nhất.");
      return toJson(result);
    } catch (Exception e) {
